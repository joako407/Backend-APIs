...
if($payload){
    ...
    if($redisInstance == null || !$redisInstance::$redis->exists('allproducts'))
    {
        // query products
        $stmt = $product->read();
        $num = $stmt->rowCount();
        ...
        if($num>0){
            ...
            if($redisInstance != null)
            {
                ...
                {
                    $redisInstance::$redis->hset('allproducts',$val['id'], serialize($val));
        ...
        $arr = $redisInstance::$redis->hgetall("allproducts");
        $products_arr = Array();
        $products_arr["records"]=array();
