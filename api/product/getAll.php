...
$product->page = isset($_GET['p']) ? $_GET['p'] : null;
...
$product->label = isset($_GET['label']) ? $_GET['label'] : null;


$redisInstance = Redis::getInstance();
if($redisInstance == null || !$redisInstance::$redis->exists("products"))
{
    ...
    if($num>0){
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)){
            ...
            if(array_search($id,array_column($products_arr['records'],'id')) === false)
            {
                ...
                //$product_item['promotion'] = $promotion->getPromotionsToProduct($product_item);            

                if($type =='p')
                    $product_item['pictures'][] = $pic_path;
                else if($type == 'd')
                    $product_item['feature_pictures'][] = $pic_path;
        ...
        {
            foreach($products_arr['records'] as $key=>$val)
            {
                $redisInstance::$redis->hset('products',$val['id'], serialize($val));
    ...
    $arr = $redisInstance::$redis->hgetall("products");
    $products_arr = Array();
    $products_arr["records"]=array();
    foreach($arr as $key=>$val)
    {
        array_push($products_arr['records'],unserialize($val));
    }

    if(count($products_arr['records']) > 0)
    {
        if(isset($_GET['brand_id']) && $_GET['brand_id'] != null)
        {
            $products_arr['records'] = array_values(array_filter($products_arr['records'], function($x){
                return $x['brand_id'] == $_GET['brand_id'];
            }));
        }
        ...
        {
            $products_arr['records'] = array_values(array_filter($products_arr['records'], function($x){
                return $x['category_id'] == $_GET['category_id'];
            }));
        }
        ...
        {
            $products_arr['records'] = array_values(array_filter($products_arr['records'], function($x){
                return $x['type_id'] == $_GET['type_id'];
            }));
        }
        
        if(isset($_GET['series_id']) && $_GET['series_id'] != null)
        {
            $products_arr['records'] = array_values(array_filter($products_arr['records'], function($x){
                return $x['series_id'] == $_GET['series_id'];
            }));
        }
        ...
        {
            $products_arr['records'] = array_values(array_filter($products_arr['records'], function($x){
                return (strpos($x['name'], $_GET['keyword']) || strpos($x['description'], $_GET['keyword']) || strpos($x['category_name'], $_GET['keyword']) || strpos($x['specification'], $_GET['keyword']));
            }));
        }
