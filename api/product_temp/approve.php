        ...
        if($product->checkExists() != 1){
            if($product->action == "edit"){
                if($product->checkProductExists()){
                    ...
                    $picture->target_id = $product->product_id;
                    $picture->readAllRemovePicture(); // assign remove pictures to picture object
                    $flag3 = $picture->delete();
                    
                    // copy the new product images
                    $product->getTempProductPicture();
                    if(!empty($product->pictures)){
                        $picture = new Picture($db,"p");
                        $picture->pictures = $product->pictures;
                        $picture->target_id = $product->product_id;
                        $picture->copyTempPicture();
                    }
                ...
                if($product->checkProductExists()){ 
                    $product->id = $product->product_id; // set id as product id since, the delete action are based on id
